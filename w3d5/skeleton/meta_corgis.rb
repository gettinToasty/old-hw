...
class MetaCorgiSnacks
  ...
  #   tasty = "get_#{method_name}_tastiness".to_sym
  #   inf = "get_#{method_name}_info".to_sym
  #   info = @snack_box.send(inf, @box_id)
  #   tastiness = @snack_box.send(tasty, @box_id)
  #   result = "#{method_name.to_s.capitalize}: #{info}: #{tastiness} "
  #   tastiness > 30 ? "* #{result}" : result
  # end
  ...
  def self.define_snack(name)
    define_method(name) do
      tasty = "get_#{name}_tastiness".to_sym
      inf = "get_#{name}_info".to_sym
      info = @snack_box.send(inf, @box_id)
      tastiness = @snack_box.send(tasty, @box_id)
